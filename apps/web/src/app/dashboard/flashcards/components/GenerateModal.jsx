...
export default function GenerateModal({ folders, onGenerate, onClose, accentColor }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => !generating && onClose()}>
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-700">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">Generate Flashcards</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Create flashcards from your uploaded documents.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Number of cards: {numCards}</label>
            <input type="range" min={3} max={30} value={numCards} onChange={(e) => setNumCards(Number(e.target.value))} className="w-full accent-indigo-600" />
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={generating} className="flex-1 px-4 py-2.5 rounded-xl text-white text-sm font-semibold disabled:opacity-50 transition-all hover:opacity-90" style={{ background: accentColor || "#6366f1" }}>
